<p align="center">
  ...
  <p align="center">
    ...
    <img src="https://img.shields.io/badge/frontend_tests-1957_passed-success?style=flat-square" alt="Frontend Tests">
...
- `raw_json`, `avatar_path`, `tools`, `tool_choice` - **never** sent. `response_format` is sent by exactly one path, the notebook's background reader, and only ever as the same fixed schema defined in this repository: the only request that carries it is that reader's own, and nothing the frontend sends can add it to any request at all. That reader does send your conversation - it is the one thing here that does, on a timer, and the Known Limitations section says so at length. This line used to say nothing sent it at all, which stopped being true in v1.1.5 when that reader shipped. `image_url` parts are built server-side **only** for images the user explicitly attached (vision models); the frontend never constructs them
...
- Logs carry numeric ids, counts and status codes - never message content, passphrases, key material, or a name you read on screen. One gap is measured rather than claimed away, because a promise nobody has counted is not a promise: crash handlers write the exception's own message, which can hold whatever the code was holding when it failed, and there are fifty such places. A scanner reads every logging call in the shipped backend, that number is written down, and it cannot grow without something going red. The gap this sentence used to name second - a voice created before the folder names were made opaque carrying the label you typed into the log when it was deleted or listed - is CLOSED: those lines now write twelve characters of a keyed hash instead, and the scanner's content ledger is empty
...
- The desktop window is given a secret at launch and every API request carries it. Loopback is not a permission boundary: without this, any program running as your user could read the whole conversation over HTTP while Elysium is open, which is exactly when the vault is unlocked. The secret travels in the URL fragment (never sent to a server, never logged), is read once and stripped from the address, and is kept in memory rather than in browser storage. It is also withheld from every subprocess Elysium starts, since the voice engine and the installer both run code this project did not write. Two routes a browser element must load directly - a stored picture and a spoken reply - accept a browser's own `Sec-Fetch-Site: same-origin` signal instead. That still refuses a hostile PAGE, because the browser sets that header itself and script on the page cannot forge it. It does not refuse a local program that sets the header by hand: `curl -H "Sec-Fetch-Site: same-origin"` passes, and this line used to say the opposite. These two routes are narrower than the rest of the gate, and that is the size of the gap
- Every outbound request THIS APP MAKES passes one check that refuses any host but the shipped provider, and refuses plain `http` to it as well, before a connection is opened. The check sits on the HTTP client every one of those requests is built with. Two things sit outside that client, and both are named here rather than in a footnote. The voice-engine installer opens its own connection to fetch a Python package manager, and the package manager it then runs reaches PyPI and the model host as separate processes; nothing on that path carries a word of your conversation, and none of it runs unless you ask for a voice. The launcher's own readiness probe is the second: on every start it asks the server it just started whether it is up, over plain `http` to `127.0.0.1` and nothing else, with empty request bodies. The list the check enforces is pinned to the shipped address rather than the configured one, and `127.0.0.1` is not on it - so any request THROUGH the client to your own machine takes the same deliberate opt-in as any other address, and the launcher's probe reaches loopback by not being one of those requests. With a proxy configured it reads the destination rather than the first hop, so the proxy cannot be used to reach elsewhere
...
>
> What it does NOT cover: the embedded browser window. WebView2 loads this app's own files from the local server and is not routed through the proxy. It makes no request to any other host - crash reporting is blocked at the filesystem level, and every API response carries a no-cache directive - `no-store` on ordinary responses, and the streaming ones keep the `no-cache` that SSE requires - but "proxy-only" is a statement about the app's outbound requests, not about the browser control.
...
- **Generated audio is session-only** - locking the vault or closing the app deletes the cached speech, the next launch clears whatever a crash or a kill left behind, and anything older than half an hour is cleared as the next reply is spoken. A session that stops talking keeps what it has already said until the next launch clears it; clearing it on the engine's own health tick instead is in the tree for v1.2.0, not in this build
...
- **Notes are written in English** - the reader is a small, cheap model and reads and writes English far better than Turkish. What you actually said is kept and shown under the English note so the paraphrase can be checked, word for word. In this build it is stored at whatever length arrived: the schema asks the model for 240 characters and a model that ignores the request is obeyed, so a long sentence goes into the vault whole and into every later prompt. Enforcing the 240 on our side is in the tree for v1.2.0. The note itself is not in your language

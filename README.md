...
- **An interrupted uploads migration leaves a snapshot nothing reports** - `app.db.premigrate.bak` is a complete encrypted copy of the vault, taken before the migration touches anything and kept whenever it does not finish cleanly. A passphrase change re-keys it, so it does not outlive the old passphrase, but no screen shows it and no button removes it: only a later clean migration does. A copy that will not open with this vault's key is moved to `app.db.premigrate.bak.unreadable-<timestamp>` rather than deleted, because it may be the only copy of an older vault. Both go when you delete the data folder

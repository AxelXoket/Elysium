...
def test_the_direct_client_keeps_an_idle_connection(client, monkeypatch):
...
def test_the_proxied_client_keeps_it_too(client, monkeypatch):
...
def test_it_is_longer_than_a_pause_between_messages(client, monkeypatch):
    """The bug in one assertion: the old default was 5 seconds."""
    monkeypatch.setattr(network_client, "get_secret", lambda name: None)
    assert _pool(network_client._build_client())._keepalive_expiry > 60.0


def test_it_stays_under_what_the_server_allows(client, monkeypatch):
...
def test_the_pool_is_sized_for_a_desktop_app(client, monkeypatch):
    monkeypatch.setattr(network_client, "get_secret", lambda name: None)
...
def test_the_environment_is_still_not_trusted(client, monkeypatch):
...
def test_a_configured_proxy_is_still_used(client, monkeypatch):
    monkeypatch.setattr(network_client, "get_secret",
                        lambda name: "http://127.0.0.1:9999")
...
def test_the_connection_stays_http_1_1(client, monkeypatch):
...
def test_the_singleton_is_still_reused(client, monkeypatch):
    monkeypatch.setattr(network_client, "get_secret", lambda name: None)

...
describe("the audio player reports what actually failed", () => {
  ...
  it("does not blame the sound card for a backend restart", async () => {
    const { VoiceStreamPlayer } = await import("@/lib/voice/streamPlayer");
    void VoiceStreamPlayer;
...
describe("the codes that fell through to the fallback", () => {
  ...
  it.each([
    // Reachable from routers/vault.py, absent from the map, two lines below
    // the sibling that was mapped correctly.
    "passphrase_too_long",
    "vault_already_initialized",
    "vault_not_initialized",
    // Listed in the contract table, missing from the map - the invariant was
    // wrong in BOTH directions.
    "cross_origin_denied",
  ])("%s says something specific", (code) => {
    expect(isKnownErrorCode(code)).toBe(true);
    expect(getErrorMessage(code)).not.toBe(FALLBACK);
  });
